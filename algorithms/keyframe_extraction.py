    ...
    while True:
        ...
        if frame_number % step == 0:
            ...
            if previous_frame is not None:
                ...
                fingerprints.append({"frame_number": frame_number, "difference": np.sum(difference)})
...
def check_transformation_resilience(video_path, transformations, step=1):
    ...
    for transform in transformations:
        ...
        for f1, f2 in zip(original_fingerprints, transformed_fingerprints):
            if f1['difference'] != f2['difference']:
                return False
    return True
...
def isSame(video_path1, video_path2, step=1, tolerance=0.01):
    ...
    # Compare the differences of corresponding frames from both videos
    for f1, f2 in zip(fingerprints1, fingerprints2):
        diff1 = f1['difference']
        diff2 = f2['difference']

        # Check if the difference between frames exceeds the tolerance
        if abs(diff1 - diff2) > tolerance * max(diff1, diff2):
            return False

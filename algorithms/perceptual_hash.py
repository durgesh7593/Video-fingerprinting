...
def generate_fingerprints(video_path, step=1):
    ...
    while True:
        ...
        if frame_number % step == 0:
            ...
            computed_hash = imagehash.phash(img)
...
def isSame(video_path1, video_path2, step=1):
    # Generate fingerprints (pHashes) for both videos
    fingerprints1 = generate_fingerprints(video_path1, step)
    fingerprints2 = generate_fingerprints(video_path2, step)

    # Check if the number of fingerprints (frames) are the same
    if len(fingerprints1) != len(fingerprints2):
        return False

    # Compare pHash values of corresponding frames in both videos
    for f1, f2 in zip(fingerprints1, fingerprints2):
        if f1['hash'] != f2['hash']:
            return False

    # If all frames match, return True
    return True
